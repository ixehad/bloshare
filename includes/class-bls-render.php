...
class BLS_Render {
    ...
    private static function button_html( $href, $label, $icon_svg, $style, $color, $target, $extra_class = '' ) {
        $btn_class = 'bls-btn bls-btn-style-' . esc_attr( $style );
        if ( $extra_class ) $btn_class .= ' ' . $extra_class;

        $inner = '';
        if ( 'icon' === $style || 'icon_text' === $style ) {
            $inner .= '<span class="bls-btn-icon" aria-hidden="true">' . wp_kses( $icon_svg, self::svg_kses() ) . '</span>';
        }
        if ( 'text' === $style || 'icon_text' === $style ) {
            $inner .= '<span class="bls-btn-label">' . esc_html( $label ) . '</span>';
        }
    ...
    public static function ai_row( $post_id = null ) {
        $settings = BLS_Settings::instance()->get_settings();
        ...
        $networks  = BLS_Settings::get_network_definitions();
        $target    = ! empty( $settings['open_new_tab'] ) ? ' target="_blank" rel="noopener noreferrer"' : '';
        $style     = $settings['ai_button_style'];
        $shape     = 'bls-shape-' . sanitize_html_class( $settings['button_shape'] );
        ...
        foreach ( $networks as $key => $def ) {
            ...
            $buttons .= self::button_html( $href, $def['label'], $def['icon'], $style, $def['color'], $target, 'bls-btn-' . esc_attr( $key ) );
        }

        if ( '' === $buttons ) return '';

        return sprintf(
            '<div class="bls-row bls-ai-row %s"><span class="bls-row-label">%s</span><div class="bls-buttons">%s</div></div>',
            esc_attr( $shape ),
    ...
    public static function social_row( $post_id = null ) {
        ...
        if ( ! empty( $settings['show_copy_link'] ) ) {
            ...
            $icon_part  = ( 'icon' === $style || 'icon_text' === $style ) ? '<span class="bls-btn-icon" aria-hidden="true">' . wp_kses( $copy_icon, self::svg_kses() ) . '</span>' : '';
            $text_part  = ( 'text' === $style || 'icon_text' === $style ) ? '<span class="bls-btn-label">Copy Link</span>' : '';
            $buttons .= sprintf(
                '<button type="button" class="bls-btn bls-btn-style-%s %s bls-btn-copy" data-kas-copy-url="%s" aria-label="Copy link to this post">%s%s</button>',
                esc_attr( $style ),
                esc_attr( $shape ),
                esc_url( $url_raw ),
                $icon_part,
        ...
        return sprintf(
            '<div class="bls-row bls-social-row %s"><span class="bls-row-label">%s</span><div class="bls-buttons">%s</div></div>',
            esc_attr( $shape ),
    ...
    /* -----------------------------------------------------------------------
     * Both rows together (used by [bls_buttons] shortcode)
     * -------------------------------------------------------------------- */
    ...
    public static function preview_rows( $settings = null ) {
        ...
        if ( ! empty( $settings['ai_enabled'] ) ) {
            ...
            foreach ( $networks as $key => $def ) {
                ...
                $href     = BLS_Render::build_url( $def['url_pattern'], $sample_url, $sample_title, $prompt );
                $buttons .= self::button_html( $href, $def['label'], $def['icon'], $style, $def['color'], $target, 'bls-btn-' . esc_attr( $key ) );
            }
            if ( '' !== $buttons ) {
                $output .= sprintf(
        ...
        if ( ! empty( $settings['social_enabled'] ) ) {
            ...
            if ( ! empty( $settings['show_copy_link'] ) ) {
                ...
                $icon_part = ( 'icon' === $style || 'icon_text' === $style ) ? '<span class="bls-btn-icon">' . wp_kses( $copy_icon, self::svg_kses() ) . '</span>' : '';
                $text_part = ( 'text' === $style || 'icon_text' === $style ) ? '<span class="bls-btn-label">Copy Link</span>' : '';
                $buttons .= sprintf(
                    '<button type="button" class="bls-btn bls-btn-style-%s %s bls-btn-copy" disabled title="Preview only">%s%s</button>',
