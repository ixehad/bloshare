 ...
 *
 * Shortcode names: [bls_ai_buttons], [bls_social_buttons], [bls_buttons]
 *
 * Auto-inject logic — per row, independent:
 *   - If [bls_ai_buttons] OR [bls_buttons] found in post → skip AI auto-inject
 *   - If [bls_social_buttons] OR [bls_buttons] found in post → skip Social auto-inject
...
class BLS_Loader {
    ...
    private function __construct() {
        // New shortcodes
        add_shortcode( 'bls_ai_buttons',     array( $this, 'sc_ai' ) );
        add_shortcode( 'bls_social_buttons', array( $this, 'sc_social' ) );
        add_shortcode( 'bls_buttons',        array( $this, 'sc_both' ) );
    ...
    public function sc_ai()     { return BLS_Render::ai_row(); }
    public function sc_social() { return BLS_Render::social_row(); }
    public function sc_both()   { return BLS_Render::both_rows(); }
    ...
    public function maybe_inject( $content ) {
        ...
        $has_ai_sc     = has_shortcode( $content, 'bls_ai_buttons' )
                      || has_shortcode( $content, 'bls_buttons' );
        $has_social_sc = has_shortcode( $content, 'bls_social_buttons' )
                      || has_shortcode( $content, 'bls_buttons' );
        ...
        $ai_row     = $inject_ai     ? BLS_Render::ai_row( $post_id )     : '';
        $social_row = $inject_social ? BLS_Render::social_row( $post_id ) : '';
    ...
    public function enqueue_front_assets() {
        ...
        wp_enqueue_script( 'bls-front', BLS_URL . 'assets/front.js',  array(), BLS_VERSION, true );
    }
}
